...
resource "google_compute_region_ssl_policy" "product_modules_server" {
  project         = var.project_id
  region          = var.location
  name            = "product-modules-server-ssl-policy"
  profile         = "RESTRICTED"
...
module "lb_frontend" {
  ...
  ssl_policy = google_compute_region_ssl_policy.product_modules_server.self_link

  # We have one already created in apps/shared-infra since it's not specific to product_modules- every
  # load balancer in the same region and VPC network uses the same proxy-only subnetwork
  create_proxy_only_subnet = false

  ssl                             = true
  managed_ssl_certificate_domains = [var.server_url]
  https_redirect                  = true
