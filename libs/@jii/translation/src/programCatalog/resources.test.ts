...
describe("program catalog translation resources", () => {
  describe("common namespace", () => {
    test("english", () => {
      expect(() =>
        commonProgramCatalogResourcesSchema.parse(commonEn),
      ).not.toThrow();
    });

    test.runIf("programs" in commonEs)("spanish", () => {
      expect(() =>
        commonProgramCatalogResourcesSchema.parse(commonEs),
      ).not.toThrow();
    });
  });
  ...
  describe.each(stateCodesWithJsonResources)(
    ...
    ({ stateCode, enPath, esPath }) => {
      ...
      const hasProgramCatalog = Object.keys(
        stateProgramCatalogResourcesSchema.shape,
      ).some((key) => key in enJson);

      test.runIf(hasProgramCatalog)("english", () => {
        expect(() =>
          stateProgramCatalogResourcesSchema.parse(enJson),
        ).not.toThrow();
      });
      ...
      test.runIf(hasProgramCatalog && isSpanishEnabled)("spanish", () => {
        const esJson = JSON.parse(readFileSync(esPath, "utf-8"));
        expect(() =>
          stateProgramCatalogResourcesSchema.parse(esJson),
        ).not.toThrow();
      });
    },
  );
