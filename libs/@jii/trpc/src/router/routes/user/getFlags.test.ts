...
import { caller, mockCtx } from "../../../test/mockUserProcedure";
import { testPrismaClient } from "../../../test/prisma";

// UserFlagId is a prisma type that has to be defined in the schema. ResidentsConfig can't
...
// they drift.
type ConfigUserFlag = keyof Required<
  (typeof residentsConfigByState)[StateCode]
>["enabledUserFlags"];
expectTypeOf<UserFlagId>().toEqualTypeOf<ConfigUserFlag>();
...
beforeEach(() => {
  vi.spyOn(testPrismaClient.userFlagInstance, "findMany").mockResolvedValue([]);
});
...
describe("getFlags", () => {
  ...
  describe("personal flags", () => {
    test("returns flags present in the DB", async () => {
      vi.mocked(testPrismaClient.userFlagInstance.findMany).mockResolvedValue([
        {
          userId,
          effectiveAt: new Date("2020-01-01"),
          flagId: "testPersonalFlag" as UserFlagId,
        },
      ]);
    ...
    test("queries by the caller's userId", async () => {
      await caller.getFlags();

      expect(testPrismaClient.userFlagInstance.findMany).toHaveBeenCalledWith(
