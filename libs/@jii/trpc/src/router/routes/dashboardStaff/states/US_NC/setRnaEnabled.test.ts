...
it("sets the last RNA to enabled", async () => {
  tk.freeze(testDate);
...
it("errors when existing RNA is already enabled", async () => {
  tk.freeze(testDate);
  ...
  const rna = await testPrismaClient.usNcRNA.create({
    data: {
      ...
      createdAt: testOldAssessmentDate,
      enabledAt: testOldAssessmentDate,
    },
  });

  await expect(() =>
    caller.setRNAEnabled({
      id: rna.id,
    }),
  ).rejects.toThrow();
});
