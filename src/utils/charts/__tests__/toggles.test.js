...
describe("test for file toggles", () => {
  ...
  it("filter dataset by metric period months", () => {
    ...
    const filterDataset = toggleMethods.filterDatasetByMetricPeriodMonths(
      dataset,
      "1"
    );
    expect(filterDataset).toEqual(expectedFilterDataset);

    const filterEmptyDataset = toggleMethods.filterDatasetByMetricPeriodMonths(
      [],
      0
    );
    expect(filterEmptyDataset).toEqual([]);

    const filterDatasetPeriodIsNotExist = toggleMethods.filterDatasetByMetricPeriodMonths(
      [],
      4
    );
  ...
  it("filter dataset by supervision type", () => {
    ...
    const filterDataset = toggleMethods.filterDatasetBySupervisionType(
      dataset,
      "PAROLE"
    );
    expect(filterDataset).toEqual(expectedFilterDataset);
  ...
  it("filter dataset by district", () => {
    ...
    const toggleFiltersTest = toggleMethods.filterDatasetByDistrict(
      dataset,
      district
    );
