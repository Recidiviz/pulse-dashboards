...
type Props = {
  ...
  title: string;
  data: PopulationProjectionTimeSeriesRecord[] | PopulationTimeSeriesRecord[];
};
...
const PopulationTimeSeriesChart: React.FC<Props> = ({
  ...
  const { gender, legalStatus, age, facility } = filtersStore.filters;
  const isProjectionChart =
  ...
  const chartTop = isProjectionChart
    ? getChartTop(projectedPopulation)
    : getChartTop(historicalPopulation);

  const projectionArea = isProjectionChart
    ? [
        {
          data: [
  ...
  const annotations = isProjectionChart
    ? [
        {
          type: "area",
  ...
  return (
    <div className="PopulationTimeSeriesChart">
      ...
      <ResponsiveXYFrame
        responsiveWidth
        // @ts-ignore
        summaries={isProjectionChart && projectionArea}
        ...
        svgAnnotationRules={(d: any) => {
          ...
          if (
            d.d.parentLine?.class.endsWith("ProjectedLine") &&
            isProjectionChart
          ) {
