...
import PagePracticesV2 from "../PagePracticesV2";
...
const practicesStoreMock = jest.fn().mockReturnValue({
  hydrate: jest.fn().mockReturnValue(jest.fn()),
})();
(useRootStore as jest.Mock).mockReturnValue({
  practicesStore: practicesStoreMock,
});

describe("PagePracticesV2 test", () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });
  ...
  it("attempts to hydrate the workflows models when not on IE11", () => {
    ...
    mount(<PagePracticesV2 />);
    expect(practicesStoreMock.hydrate).toHaveBeenCalled();
  });

  it("does not attempt to hydrate the workflows models when on IE11", () => {
    (isIE11 as jest.Mock).mockReturnValue(true);
    mount(<PagePracticesV2 />);
    expect(practicesStoreMock.hydrate).toHaveBeenCalledTimes(0);
  });
});
