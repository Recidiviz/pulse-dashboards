...
```bash
gh api repos/Recidiviz/pulse-dashboards/pulls/<pr_number>/reviews --jq '[.[] | select(.user.login == "<username>")] | last'
```

Then fetch comments from that review using the review ID:

```bash
gh api repos/Recidiviz/pulse-dashboards/pulls/<pr_number>/reviews/<review_id>/comments
```
...
```bash
gh api graphql -f query='query { repository(owner: "Recidiviz", name: "pulse-dashboards") { pullRequest(number: <pr_number>) { reviewThreads(first: 100) { nodes { id isResolved comments(first: 1) { nodes { databaseId path line body author { login } } } } } } } }'
```
...
gh api repos/Recidiviz/pulse-dashboards/pulls/comments/<comment_databaseId>/reactions -X POST -f content="+1"
...
gh api repos/Recidiviz/pulse-dashboards/pulls/<pr_number>/comments/<comment_databaseId>/replies -X POST -f body="<message>"
