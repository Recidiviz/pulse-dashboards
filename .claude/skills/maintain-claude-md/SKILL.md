...
> "I notice the CLAUDE.md says the backend runs on port 3000 but it's actually on port 3001. I'm correcting that."
...
**Signal**: While reading code, you notice that a shared library has moved from `libs/datatypes/` to `libs/common/` but CLAUDE.md still references the old path

**Action**:

1. Recognize this is outdated documentation
2. Mention: "I notice the CLAUDE.md has the wrong library path. I'm correcting it."
3. Update the path in CLAUDE.md

### Scenario 2: You make a code change that affects documentation

**Signal**: You rename an nx target from `dev` to `dev-spa` in `apps/staff/project.json`

**Action**:

1. Complete the code change
2. Check if CLAUDE.md references the old target name
3. If yes, say: "I'm updating CLAUDE.md to reflect the renamed nx target"
4. Update all references to the old target name
...
- [Maintain Skill Files](./../maintain-skill-files/SKILL.md) - Skill for maintaining SKILL files
