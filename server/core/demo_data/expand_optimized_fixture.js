...
/**
 * Uses a copied and pasted version of optimizedMetricFileParser from src/api/metrics
 * to create the expanded Json version of the optimized fixture file.
 * Usage: node expand_optimized_fixture.js some_file.txt
 * Produces: some_file.json
 */
...
const metadataFilePath = path.resolve(__dirname, `./${fileKey}.metadata.json`);

const metadataContents = fs.readFileSync(metadataFilePath);
const metadata = JSON.parse(metadataContents);

const fixtureContents = fs.readFileSync(filePath).toString();
const dataPoints = expandMetricRepresentation(fixtureContents, metadata);
