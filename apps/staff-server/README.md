...
The legacy Node/Express backend for the staff dashboard. It mainly serves the older flat-file-based endpoints for our earlier dashboards (Pathways/Lantern metrics); newer backend functionality can be found in other `/apps` projects in this repo or in Python. Shared filtering and optimization logic lives in the [`~staff-shared-filters`](/libs/staff-shared-filters) library.
...
You can run the backend development server on its own using `nx dev staff-server`.
...
You can launch in offline mode locally via `nx offline staff`, which will run both the frontend and backend. Use `nx offline staff-server` to start the backend only.
...
Example usage:
`apps/staff-server/src/server/core/demo_data/download_fixture_with_metadata.sh gs://some-data-bucket/US_ID/vitals_summaries.txt`

## Running the server in Offline mode with the Python backend

The staff app's Python backend can be run in offline mode, using some configuration and fixture data that is shared with this server's offline mode. It does not run by default in the staff app's offline mode. If you do need to run it, most of the time you will want to start the frontend and backends all together; refer to the documentation in `/apps/staff` for instructions. To run the Python backend on its own, run `nx python-backend-offline staff-server`.
...
To generate new fixture data, while the backend is running run `./apps/staff-server/tools/generate_pathways_fixtures.sh`.

To load fixture data from a GCS bucket, run `gsutil -m cp "gs://$BUCKET_NAME/US_OZ/*.csv" ./apps/staff-server/src/server/core/demo_data/`.
...
We use [Memorystore for Redis](https://cloud.google.com/memorystore/docs/redis/redis-overview) as an external cache for the backend API. When developing locally, the `redis-server` will be started with the `nx dev staff-server` or `nx offline staff-server` scripts.
