...
import { formatStateAddress, formatYearsMonthsFromNow } from "../formatStrings";
...
describe("formatStrings", () => {
  ...
  describe("formatStateAddress", () => {
    it("handles normal addresses", () => {
      expect(
        formatStateAddress("123 MAIN ST, ANYTOWN, TX 12345"),
      ).toMatchInlineSnapshot(`"123 Main St, Anytown, TX 12345"`);
      expect(
        formatStateAddress("2 SOME LONG AVENUE, LONGER CITY NAME, TX 12345"),
      ).toMatchInlineSnapshot(
        `"2 Some Long Avenue, Longer City Name, TX 12345"`,
      );
      expect(
        formatStateAddress("99 COUNTY HWY 999 ANYTOWN TX 12345"),
      ).toMatchInlineSnapshot(`"99 County Hwy 999 Anytown TX 12345"`);
      expect(
        formatStateAddress("100 FM 100 ANYTOWN TX 12345"),
      ).toMatchInlineSnapshot(`"100 FM 100 Anytown TX 12345"`);
      expect(
        formatStateAddress("1000 5TH DRIVE APT#123, ANYTOWN, TX, 12345"),
      ).toMatchInlineSnapshot(`"1000 5th Drive Apt#123, Anytown, Tx, 12345"`);
    });

    it("reformats ZIP codes at the end of addresses", () => {
      expect(
        formatStateAddress("123 MAIN ST, ANYTOWN, TX 123450000"),
      ).toMatchInlineSnapshot(`"123 Main St, Anytown, TX 12345"`);
      expect(
        formatStateAddress("123 MAIN ST, ANYTOWN, TX 12345-6789"),
      ).toMatchInlineSnapshot(`"123 Main St, Anytown, TX 12345"`);
    });

    it("removes pipes", () => {
      expect(
        formatStateAddress("123 MAIN ST|APT#3, ANYTOWN, TX 12345"),
      ).not.toContain("|");
    });

    it("capitalizes state abreviations", () => {
      expect(
        formatStateAddress("123 MAIN ST|APT#3, ANYTOWN, id 12345"),
      ).toMatchInlineSnapshot(`"123 Main St, Apt#3, Anytown, ID 12345"`);

      expect(
        formatStateAddress("123 MAIN ST|APT#3, ANYTOWN, Tx 12345"),
      ).toMatchInlineSnapshot(`"123 Main St, Apt#3, Anytown, TX 12345"`);
    });
    ...
    it("removes words before the first all-numbers word", () => {
      ...
      expect(
        formatStateAddress("ABC123 HOMES #456, 123 MAIN ST, ANYTOWN, TX 12345"),
      ).toMatchInlineSnapshot(`"123 Main St, Anytown, TX 12345"`);
      ...
      expect(
        formatStateAddress("EXAMPLE'S RV PARK 123 MAIN ST, ANYTOWN, TX 12345"),
      ).toMatchInlineSnapshot(`"123 Main St, Anytown, TX 12345"`);
    });

    it("adds spaces only where needed after periods and commas", () => {
      expect(
        formatStateAddress("123 S.MAIN ST, ANYTOWN, TX 12345"),
    ...
    it("removes phone numbers in different formats", () => {
      ...
      expect(
        formatStateAddress("123 MAIN ST 555 555 5555 ANYTOWN, TX 12345"),
      ).toMatchInlineSnapshot(`"123 Main St Anytown, TX 12345"`);
      expect(
        formatStateAddress("123 MAIN ST 555.555.5555 ANYTOWN, TX 12345"),
      ).toMatchInlineSnapshot(`"123 Main St Anytown, TX 12345"`);
      ...
      expect(
        formatStateAddress("123 MAIN ST, 5555555555, ANYTOWN, TX 12345"),
      ).toMatchInlineSnapshot(`"123 Main St, Anytown, TX 12345"`);
      expect(
        formatStateAddress("123 MAIN ST, 1 555 555 5555, ANYTOWN, TX 12345"),
      ).toMatchInlineSnapshot(`"123 Main St, Anytown, TX 12345"`);
      expect(
        formatStateAddress("123 MAIN ST, #555-555-5555, ANYTOWN, TX 12345"),
      ).toMatchInlineSnapshot(`"123 Main St, Anytown, TX 12345"`);
      expect(
        formatStateAddress("123 MAIN ST, CELL#555-555-5555, ANYTOWN, TX 12345"),
      ).toMatchInlineSnapshot(`"123 Main St, Anytown, TX 12345"`);
    });
