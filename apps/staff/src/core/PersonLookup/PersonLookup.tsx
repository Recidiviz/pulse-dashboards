...
export const PersonLookup = observer(function PersonLookup() {
  ...
  // Debounce: Sets a 200ms timeout before running the search, so the search doesn't fire on every
  // keystroke — it waits until the user pauses typing
  useEffect(() => {
    const timeoutId = setTimeout(() => {
      if (searchValue.trim().length >= 2) {
        const searchResults = workflowsStore.searchPersons(searchValue);
