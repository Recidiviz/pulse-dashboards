...
const formatMeetingDuration = ({ startDate, endDate }: { startDate: Date | null, endDate: Date | null }) => {
  if (!startDate) {
  ...
  const duration = differenceInSeconds < 60 
...
const MeetingScreen = () => {
  ...
  const bottomViewStyles = useAnimatedStyle(() => ({
    paddingTop: interpolate(scrollY.value,
      [0, 50, 100, 101],
      [202, 136, 84, 152],
      "clamp")
  }));
  ...
  const meetingDate = meetingDetails?.startTime ? formatMeetingStartDate(meetingDetails.startTime) : "";
    ...
    <SafeAreaView className="flex-1">
      ...
      >     
        <View className="flex flex-row items-center justify-between h-16">
          ...
          <Animated.View className="flex flex-col items-center gap-0.5" style={[topDateStyle]}>
        ...
        <View className="flex flex-col gap-4">
          ...
          <Animated.View className="flex flex-row items-baseline gap-x-2 gap-y-1 flex-wrap" style={[userNameStyle]}>
          ...
          <Animated.View className="flex flex-col gap-2" style={[bottomDateStyle]}>
      ...
      >
        ...
        >
          {activeTab === Tab.Notes && <MeetingNotesTab notes={meetingDetails?.notes} />}
          {activeTab === Tab.Transcription && <MeetingTranscriptionTab transcription={meetingDetails?.transcription} />}
        </Animated.ScrollView>
      </Animated.View>
    </SafeAreaView>
  );
