 ...
 */
export function extractOriginalCSS(element: Element, options: ExtractCSSOptions = {}) {
  const {
      ...
      try {
        if (rule instanceof CSSStyleRule) {
          ...
          if (mediaRules.length > 0) {
            matchingRules.add(`@media ${mediaRule.conditionText} {\n${mediaRules.join('\n')}\n}`);
          }
...
export function extractCompleteCSS(
  ...
  const elements = options.includeChildren !== undefined && !options.includeChildren
    ? [element]
    : [element, ...element.querySelectorAll('*')];
