...
const ClientProfilePage = () => {
  ...
  } = $api.useQuery("get", "/clients/{client_pseudo_id}/intakes", {
    params: {
      path: { client_pseudo_id: clientId },
    },
    headers: {
      ...
      <div className="w-full p-6 md:px-6 md:py-2 flex-col items-center gap-2 inline-flex bg-[#f9fafa] min-h-[calc(100vh-65px)]">
              ...
              <PrimaryButton
                buttonText={
                  isResettingInProgress ? (
                    <div className="flex items-center gap-2">Resetting...</div>
                  ) : (
                    <div className="flex items-center gap-2">Reset Client</div>
                  )
                }
                ...
                onClick={() =>
                  handleResetClient(clientData?.pseudonymized_client_id, () =>
                    location.reload(),
                  )
                }
              ...
              <PrimaryButton
                buttonText={
                  isDeletingInProgress ? (
                    <div className="flex items-center gap-2">Deleting...</div>
                  ) : (
                    <div className="flex items-center gap-2">Delete Client</div>
                  )
                }
        ...
        <div className={"flex flex-col w-full px-2 md:px-14 max-w-7xl flex-1"}>
          <div className={"flex mt-4 w-full justify-center"}>
            <ClientMetadata clientData={clientData} />
          </div>
          <div className="w-full justify-between items-center gap-2 inline-flex mt-12">
            <div className="justify-center text-black text-2xl font-medium font-['Public_Sans'] leading-7">
              Assessments
            </div>
          ...
          <div
            className={`flex flex-col my-10 gap-10 w-full justify-center ${intakeList?.length == 0 && "flex-1"}`}
          >
            {intakeList && intakeList?.length > 0 ? (
              intakeList.map((intakeInfo) => (
                <IntakeAssessment
                  key={intakeInfo.id}
              ...
              >
                <div className="w-full flex flex-col gap-4 text-center md:p-14">
                  <div className="text-center text-[#012322] font-['Public_Sans'] text-2xl font-medium leading-[120%] tracking-[-0.48px]">
                    There are no active or enabled assessments
                  </div>
                  <div className="mt-2 text-center text-[rgba(43,84,105,0.85)] font-['Public_Sans'] text-lg font-medium leading-[120%] tracking-[-0.36px]">
                    Enable a new assessment for{" "}
                    {clientData?.full_name?.given_names} by clicking <br /> the
                    “Enable New Assessment” button in the top right.
                  </div>
                </div>
              </div>
            )}
