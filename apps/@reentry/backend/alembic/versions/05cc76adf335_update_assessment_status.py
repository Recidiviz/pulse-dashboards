...
Revision ID: 05cc76adf335
Revises: 0d0e9f7b252d
Create Date: 2025-04-08 21:59:43.929169
...
revision: str = '05cc76adf335'
down_revision: Union[str, None] = '0d0e9f7b252d'
...
def upgrade() -> None:
    ...
    # First drop the view that depends on the execution.status column
    op.execute("""DROP VIEW IF EXISTS client_view;""")

    # Create the enum type and alter the column using raw SQL (one at a time)
    op.execute("CREATE TYPE execution_status_enum AS ENUM ('not_started', 'pending', 'in_progress', 'completed', 'failed')")
    op.execute("ALTER TABLE execution ALTER COLUMN status TYPE execution_status_enum USING status::execution_status_enum")
...
def downgrade() -> None:
    ...
    # Convert back to VARCHAR and drop the enum type using raw SQL (one at a time)
    op.execute("ALTER TABLE execution ALTER COLUMN status TYPE VARCHAR USING status::TEXT")
    op.execute("DROP TYPE execution_status_enum")

    # Recreate the client_view (same as in 0d0e9f7b252d)
    op.execute("""
    CREATE OR REPLACE VIEW client_view AS
    -- INTAKE CLIENTS (earliest in process): Order values 10-30
    SELECT
