...
def check_access(
    ...
    If no match is found and the user is a zero-caseload user, fall back to checking
    location-based access. Non-zero-caseload users can only access directly assigned clients.
    """
    ...
    # Only zero-caseload users get the location-based fallback
    if is_zero_caseload_user:
        ...
        # If client does not exist, log and raise 404
        if not client_record:
            logger.warning(f"nonexistent or invalid client {client_pseudo_id}")
            raise HTTPException(status_code=404, detail="Client not found")

        # Step 3: Gather all staff locations (staff_record is optional)
        locations = []
        if staff_record and staff_record.locations:
