...
async def test_register_client(connection_manager, mock_redis_client, mock_socketio):
    ...
    await connection_manager.register_client(client_id, sid)

    # Verify Redis interactions for new key
    mock_redis_client.set.assert_called()

    # Verify Socket.IO interactions
    mock_socketio.enter_room.assert_called_once_with(sid, f"client_{client_id}")
    mock_socketio.save_session.assert_called_once_with(sid, client_id)
...
async def test_register_client_existing_connection(
    ...
    result = await connection_manager.register_client(client_id, sid)
...
async def test_disconnect_client(connection_manager, mock_redis_client, mock_socketio):
    ...
    # Mock client lookup
    connection_manager.get_client_id_by_sid = AsyncMock(return_value=client_id)
    ...
    # Verify Socket.IO interactions
    mock_socketio.leave_room.assert_called_once_with(sid, f"client_{client_id}")


@pytest.mark.asyncio
async def test_get_client_id_by_sid(
    connection_manager, mock_redis_client, mock_socketio
):
    ...
    result = await connection_manager.get_client_id_by_sid(sid)
...
async def test_get_client_sid(connection_manager, mock_redis_client):
    ...
    result = await connection_manager.get_client_sid(client_id)
...
@pytest.mark.asyncio
async def test_register_client_twice_same_client_id(
    connection_manager, mock_redis_client, mock_socketio
):
    """Test registering a client with the same client_id twice should fail."""
    client_id = "test_client"
    sid1 = "socket_one"
    sid2 = "socket_two"
    ...
    mock_redis_client.get.return_value = None
    result1 = await connection_manager.register_client(client_id, sid1)
    ...
    # Second connection with same client_id - should disconnect first one
    result2 = await connection_manager.register_client(client_id, sid2)
