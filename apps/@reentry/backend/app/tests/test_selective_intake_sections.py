...
class TestSelectiveIntakeSectionsSeeding:
    ...
    async def test_seed_sections_selective_creates_sections(self, async_session):
        ...
        # Verify LSIR sections were created
        result = await async_session.exec(
            select(IntakeSection).where(IntakeSection.assessment_type == "lsir")
        )
        lsir_sections = result.all()
        assert len(lsir_sections) > 0

        # Verify ORAS_RT sections were created
        result = await async_session.exec(
            select(IntakeSection).where(IntakeSection.assessment_type == "oras_rt")
        )
        oras_sections = result.all()
        assert len(oras_sections) > 0

        # Verify sections have correct order
        for i, section in enumerate(lsir_sections):
    ...
    async def test_seed_sections_selective_updates_content(self, async_session):
        ...
        # Mock the constants to simulate content changes
        mock_sections_lsir = [
            {
                "title": "Employment",
            ...
            },
            {
                "title": "Education",
                "description": "Let's talk about your educational background",
                "required_information": "Educational history and goals",
            },
        ]

        with patch(
            "app.manage.intake.get_intake_sections_for_assessment_type"
        ) as mock_get_sections:
            # Configure mock to return modified content for LSIR
            def mock_get_sections_side_effect(assessment_type):
                if assessment_type == "lsir":
                    return mock_sections_lsir
                else:
                    return []  # Empty for ORAS_RT to simplify test

            mock_get_sections.side_effect = mock_get_sections_side_effect
